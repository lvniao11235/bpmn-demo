...
import { getCustomRenders } from '@/components/bpmn/renders'
...
export default function BpmnRenderer(
    ...
    const customerRender = {
        drawElement: function(parentGfx, element) {
            var attrs = {
                fill: getFillColor(element, defaultFillColor),
                stroke: getStrokeColor(element, defaultStrokeColor)
            };
        ...
        drawImage: function(parentGfx, element, imgUrl) {
            ...
            if (element.businessObject.name && element.businessObject.name.length > 0) {
