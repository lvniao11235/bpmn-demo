...
function isMessageFlowSource(element) {
    return (
        ...
        !is(element, 'bpmn:BoundaryEvent') && (!is(element, 'bpmn:Event') || (
...
function isMessageFlowTarget(element) {
    return (
        is(element, 'bpmn:InteractionNode') &&
        !isForCompensation(element) && (!is(element, 'bpmn:Event') || (
...
function canResize(shape, newBounds) {
    if (is(shape, 'bpmn:SubProcess')) {
        return (
            isExpanded(shape) && (!newBounds || (newBounds.width >= 100 && newBounds.height >= 80))
        );
    }
